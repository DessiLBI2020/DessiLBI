...
def conv_bn(inp, oup, stride):
    return nn.Sequential(
        torch.nn.utils.weight_norm(nn.Conv2d(inp, oup, 3, stride, 1, bias=False)),
        nn.BatchNorm2d(oup),
...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv = nn.Sequential(
            torch.nn.utils.weight_norm(nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)),
            nn.BatchNorm2d(planes),
            nn.ReLU(inplace=True),
            torch.nn.utils.weight_norm(nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)),
            nn.BatchNorm2d(planes),
        )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.classifier = nn.Sequential(torch.nn.utils.weight_norm(nn.Linear(64, num_classes)))
