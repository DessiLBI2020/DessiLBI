...
class SLBI(Optimizer):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if  'z_buffer' in param_state:
                    ...
                    elif len(p.data.size()) == 4:# recasle the group norm!
                        param_state['gamma_buffer'] = kappa * self.shrink_group(param_state['z_buffer'],lam=conv_lambda)
                    else:
                        pass
            ...
            for p in group['params']:
                ...
                if  'z_buffer' in param_state:
                    if len(p.data.size()) == 1:
                        ...
                        print('number of selected weights:', num_selected_units)
                        print('number of all weights:',  p.data.size()[0] * p.data.size()[1] * p.data.size()[2] * p.data.size()[3])
                        print('ratio of selected weights:', num_selected_units/(p.data.size()[0] * p.data.size()[1] * p.data.size()[2] * p.data.size()[3]))
                        print('###############################################')
    ...
    def shrink_group(self, ts,lam=1.0): # sqrt filter size
		# shrinkage for 4-d weight(conv layer)
        ts_reshape = torch.reshape(ts,(ts.shape[0],-1))
        ts_norm = torch.norm(ts_reshape,2,1)*(1/math.sqrt(ts_reshape.shape[1]))
        ts_shrink = torch.max(torch.zeros_like(ts_norm),torch.ones_like(ts_norm) - torch.div(lam*torch.ones_like(ts_norm),ts_norm))
